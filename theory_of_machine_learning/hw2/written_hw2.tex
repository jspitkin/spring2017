...
The text defines a convex function as such. Let $C$ be a convex set. A function $f$ : $C \rightarrow \mathbb{R}$ is convex if for every $\mathbf{u}, \mathbf{v} \in C$ and $\alpha \in [0, 1]$,

$$f(\alpha \mathbf{u} + (1 - \alpha) \mathbf{v}) \leq \alpha f(\mathbf{u}) + (1 - \alpha)f(\mathbf{v})$$

Let us apply this definition to $x, z \in K$ where $f(x) = 2$ and $\vert \vert \nabla f(x) \vert \vert = 1$.

$$f(\alpha \mathbf{x} + (1 - \alpha) \mathbf{z}) \leq \alpha f(\mathbf{x}) + (1 - \alpha)f(\mathbf{z})$$

We know the diameter of the convex set $K$ is 1 and since we are interested in a lower bound, we will select the most optimal point in $K$ which is $z = x - 1$ (as $f$ has a positive gradient). Replacing $z = x - 1$ and  $f(x) = 1$ in the definition

$$f(\alpha \mathbf{x} + (1 - \alpha) (\mathbf{x} - 1)) \leq \alpha * 2 + (1 - \alpha)f(\mathbf{z})$$

Take $\alpha = 0$ as we are interested in $f(z)$

$$f(0 * \mathbf{x} + (1 - 0) (\mathbf{x} - 1)) \leq 0 * 2 + (1 - 0)f(\mathbf{z})$$

$$f(x - 1) \leq f(\mathbf{z})$$

We know $\vert \vert \nabla f(x) \vert \vert = 1$ so it follows that $f(x - 1) = f(x) - 1 = 1$

$$1 \leq f(\mathbf{z})$$

Thus min$_{z \in K}$ $f(z)$ is lower bounded by 1.
...
Take $n$ points $(a_1, b_1), (a_2, b_2), ... , (a_n, b_n)$ chosen from $[-1, 1]^2$ (uniformly). Given the uniform distribution and large enough $n$, we can expect the point $(x, y)$ that minimizes $f(x, y)$ to be $(0, 0)$. More clearly, we can expect $\sum_{i = 1}^n a_i = 0$ and $\sum_{i = 1}^n b_i = 0$ and from part a it follows that the optimal point will be $(x, y) = (0, 0)$. \\

At each iteration of gradient descent, we will take a (dependent on $\eta$) step towards the optimal point. In problem 6b we proved that if $f$ is convex and we have an optimal point $x^*$, then $- \nabla f(x)$ points \textit{towards} $x^*$. \\

Depending on our choice of $\eta$ in $0 < \eta < 1$, we may experience oscillation around the optimum point.
...
\fbox{ \parbox{1\linewidth}{
Consider a fixed $\eta = 1/2$. For each iteration of SGD, a random point $(a_i, b_i)$ from the $n$ points is selected and $\mathbf{w} = (x, y)$ is updated with the following rule

$$\mathbf{w}^{(t + 1)} = \mathbf{w}^{(t)} - \eta * \nabla f_i(\mathbf{w}^{(t)})$$

We calculate $\nabla f_i(\mathbf{w}) = (2(x - a_i), 2(y - b_i))$. Giving the following when updating $\mathbf{w}$ each iteration

$$(x, y)^{(t + 1)} = (x, y)^{(t)} - \frac{1}{2} * (2(x - a_i), 2(y - b_i))^{(t)}$$

$$(x, y)^{(t + 1)} = (x, y)^{(t)} - (x - a_i, y - b_i)^{(t)}$$

$$(x, y)^{(t + 1)} = (x^{(t)} - x^{(t)} + a_i, y^{(t)} - y^{(t)} + b_i)$$

$$(x, y)^{(t + 1)} = (a_i, b_i)$$

By picking $\eta = 1/2$, each iteration the weight vector will be updated to the current point $(a_i, b_i)$. It's clear that we don't any hope getting arbitrarily close the optimum. We will show this experimentally in part d.
...
In experiment 1  we can see the poor results described in part c. In experiment 2 with a dropping step size, we achieve better results and move towards the optimum point.
...
\part{b} A simple way to overcome this is to avoid storing probabilities, but instead to maintain the number of mistakes $m_t^{(i)}$. Prove how this suffices to recover the probabilities $p_t^{(i)}$j (assuming infinite precision arithmetic).
