...
\fbox{ \parbox{1\linewidth}{
\textit{claim:} A finite hypothesis class $\mathcal{H}$ has a VC dimension of at most $log_2 \vert \mathcal{H} \vert$.\\

\textit{proof by contradiction:} Assume that the opposite $VC(\mathcal{C}) > log_2|C|$ is true. \\
...
Given that X is of size d, it holds that $0 \leq VC(C) \leq d$ as $\mathcal{H}$ can shatter \textbf{at most} d points (the size of the instance space). Visiting the initial assumption:

$$VC(\mathcal{H}) > \log_2|H|$$
$$VC(\mathcal{H}) > d$$
...
Showing the VC($\mathcal{H}$) $\leq O(m + nlog(n))$. Thus we can see $VC(\mathcal{H})$ is upper-bounded by $O((m+n)\log ⁡n)$.
} }
...
\begin{figure}[H]
  \centerline{\includegraphics[width=0.5\linewidth]{13b.png}}
\end{figure}
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \node[]                     (A)                    {$x$};
  \node[draw, circle]         (B) [above right of=A] {$y_1$};
  \node[draw, circle]         (C) [right of=A] {$y_2$};
  \node[draw, circle]         (D) [below right of=A] {$y_3$};
  \node[]					  (E) [right of=C] {$f$};


  \path (A) edge              node {$w_1^{(1)}$} (B)
  		(A) edge			  node {$w_2^{(1)}$} (C)
  		(A) edge			  node {$w_3^{(1)}$} (D)
        (B) edge              node {$w_1^{(2)}$} (E)
        (C) edge              node {$w_2^{(2)}$} (E)
        (D) edge              node {$w_3^{(2)}$} (E);
\end{tikzpicture}
\caption{A network with n = 3 nodes in a hidden layer}
\end{figure}

Where $y_1$, $y_2$, $y_3$, and $f$ are defined as such

$$y_1(x) = max \{ 0,w_1^{(1)}x + b_1 \} \ \ \ \ y_2(x) = max \{ 0,w_2^{(1)}x + b_2 \} \ \ \ \ y_3(x) = max \{ 0,w_3^{(1)}x + b_3 \}$$
$$f(y_1, y_2, y_3) = max \{ 0,w_1^{(2)}y_1 + w_2^{(2)}y_2 + w_3^{(3)}y_3 + b \}$$
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
  ...
  \node[]                     (A)                    {$x$};
  \node[draw, circle]         (B) [above right of=C] {$0$};
  \node[draw, circle]       (F) [right of=B] {$0$};
  \node[draw, circle]       (G) [right of=F] {$0$};
  \node[draw, circle]         (C) [right of=A] {$y_1$};
  \node[draw, circle]         (H) [right of=C] {$y_2$};
  \node[draw, circle]         (D) [right of=H] {$y_3$};
  \node[draw, circle]         (I) [below right of=C] {$0$};
  \node[draw, circle]         (J) [right of=I] {$0$};
  \node[draw, circle]         (K) [right of=J] {$0$};
  \node[]         (L) [right of=K] {$g$};
